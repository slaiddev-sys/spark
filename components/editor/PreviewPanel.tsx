...
export default function PreviewPanel({ frames, selectedFrameId, onSelectFrame, deviceMode, onUndo, onRedo, canUndo, canRedo, projects, currentProject, onSelectProject, onCreateProject, onRenameProject, onDeleteProject, user, forcePresentationMode = false, isPricingModalOpen, setIsPricingModalOpen }: PreviewPanelProps) {
    ...
    setFramePositions(prev => {
        ...
        frames.forEach(frame => {
            if (!newPositions[frame.id]) {
                ...
                newPositions[frame.id] = { x: maxX - width / 2, y: -height / 2 }
                    ...
                    <div className="flex items-center space-x-1 flex-shrink-0">
                        ...
                        {editingProjectId !== project.id && (
                            <button
                                ...
                                className="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-white transition-all p-1.5 rounded-md hover:bg-gray-700"
                        ...
                        {editingProjectId !== project.id && (
                            <button
                                ...
                                className="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-500 transition-all p-1.5 rounded-md hover:bg-gray-700"
            ...
            {frames.map((frame) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Loading Badge (Outside Mockup) */}
                  {!frame.content && !frame.locked && (
                    <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 z-30">
                      <div className="bg-[#5b7cfa] text-white px-4 py-2 rounded-full flex items-center space-x-2 shadow-lg animate-in fade-in slide-in-from-bottom-2 duration-300">
                        <svg className="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        <span className="text-sm font-medium">Preparing Design</span>
                      </div>
                    </div>
                  )}
                </div>

      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Pro Frames Placeholders (Locked) - Only show when at least 3 frames are generated */}
            {frames.length >= 3 && Array.from({ length: 3 }).map((_, i) => {
               ...
               // For now, let's just place them arbitrarily to the right.
               
               let maxX = 0
               const existingPositions = Object.values(framePositions)
               if (existingPositions.length > 0) {
                    maxX = Math.max(...existingPositions.map(p => p.x)) + 400
               }
               // Add offset for each locked frame
               const lockedX = maxX + (i * 500)
               const lockedY = 0
               
               return (
               <div 
                 key={`locked-${i}`} 
                 className="bg-white shadow-2xl overflow-hidden border border-gray-800 absolute"
                 style={{ 
                    width: deviceMode === 'mobile' ? '375px' : '1280px',
                    height: deviceMode === 'mobile' ? '812px' : '800px',
               ...
               >
                 ...
                 <div className="w-full h-full p-6 space-y-8 bg-[#f8f9fa] opacity-50 filter blur-sm select-none pointer-events-none flex flex-col">
                 ...
                 <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-md z-20">
                    <div className="bg-white/10 border border-white/20 p-8 rounded-3xl flex flex-col items-center backdrop-blur-xl shadow-2xl">
                      ...
                      <p className="text-gray-200 text-sm mb-6 text-center max-w-[220px] leading-relaxed">
                        Upgrade to Spark Pro to unlock the complete 6-screen user journey and export code.
                      </p>
                      <button 
                        onClick={(e) => {
                          e.stopPropagation()
